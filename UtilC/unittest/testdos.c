...
static void create_temp_file()
{
	...
	bool ok = dos1_fcreate(TEMP_FILE);
	ASSERT_EQUAL(ok, true, ERROR);
	RETW wrsize = dos1_fwrite(TEMP_FILE, strlen(TEMP_FILE));
	ASSERT_EQUAL(wrsize, strlen(TEMP_FILE), ERROR);
	ok = dos1_fclose();
	ASSERT_EQUAL(ok, true, ERROR);
}
...
static void create_file(char *text)
{
	...
	bool ok = dos1_fcreate(TEMP_FILE);
	ASSERT_EQUAL(ok, true, ERROR);
	RETW wrsize = dos1_fwrite(text, strlen(text));
	ASSERT_EQUAL(wrsize, strlen(text), ERROR);
	ok = dos1_fclose();
	ASSERT_EQUAL(ok, true, ERROR);
}
...
void test_get_program_path()
{
	...
	//BDD then
	ASSERT(resultp != NULL, ERROR);
	ASSERT(strcmp(resultp, "A:\\TESTDOS.COM")==0 ||
		   strcmp(resultp, "A:\\")==0, resultp);
...
void test_fopen()
{
	...
	//BDD when
	resultbool = dos1_fopen(TEMP_FILE);
...
void test_fclose()
{
	...
	//BDD when
	dos1_fopen(TEMP_FILE);
	resultbool = dos1_fclose();
...
void test_fcreate()
{
	...
	//BDD when
	resultbool = dos1_fcreate(TEMP_FILE);
...
void test_remove()
{
	...
	//BDD when
	resultbool = dos1_remove(TEMP_FILE);
...
void test_remove_FAILS()
{
	...
	//BDD when
	resultbool = dos1_remove(NO_FILE);
...
void test_fread()
{
	...
	//BDD when
	dos1_fopen(TEMP_FILE);
	result16 = dos1_fread(heap_top, strlen(TEMP_FILE));
	dos1_fclose();
...
void test_fread_EOF()
{
	...
	//BDD when
	dos1_fopen(TEMP_FILE);
	result16 = dos1_fread(heap_top, strlen(TEMP_FILE)+20);
	dos1_fclose();
...
void test_fwrite()
{
	...
	//BDD when
	dos1_fcreate(TEMP_FILE);
	result16 = dos1_fwrite(TEMP_FILE, strlen(TEMP_FILE));
	dos1_fclose();
...
void test_fwrite_FAILS()
{
	...
	//BDD when
	result16 = dos1_fwrite(TEMP_FILE, 10);
...
void test_fgets()
{
	...
	//BDD when
	dos1_fopen(TEMP_FILE);
	char *result = dos1_fgets(heap_top, strlen(TEMP_FILE)+1); //gets the content + '\0'
	dos1_fclose();
...
void test_fgets_CRLF()
{
	...
	//BDD when
	dos1_fopen(TEMP_FILE);
	resultp = dos1_fgets(heap_top, 16);
	dos1_fclose();
...
void test_fgets_FAILS()
{
	...
	//BDD when
	resultp = dos1_fgets(heap_top, 100);
...
void test_fseek_SET()
{
	...
	//BDD when
	dos1_fopen(TEMP_FILE);
	result32 = dos1_fseek(5, SEEK_SET);
	dos1_fread(heap_top, 13);
	dos1_fclose();
...
void test_fseek_CUR()
{
	...
	//BDD when
	dos1_fopen(TEMP_FILE);
	dos1_fread(heap_top, 2);
	result32 = dos1_fseek(3, SEEK_CUR);
	dos1_fread(heap_top, 13);
	dos1_fclose();
...
void test_fseek_END()
{
	...
	//BDD when
	dos1_fopen(TEMP_FILE);
	result32 = dos1_fseek(-3, SEEK_END);
	dos1_fread(heap_top, 13);
	dos1_fclose();
...
void test_fseek_FAILS()
{
	...
	//BDD when 1
	result32 = dos1_fseek(3, SEEK_SET);
	ASSERT_EQUAL(result32, 3, ERROR);

	//BDD when 2
	dos1_fopen(TEMP_FILE);
	result32 = dos1_fseek(3, 0xff);
	dos1_fclose();
...
void test_fflush()
{
	...
	//BDD when
	dos1_fopen(TEMP_FILE);
	dos1_fseek(0, SEEK_END);
	dos1_fwrite(heap_top, 10);
	dos1_fflush();
	dos1_fclose();
...
void test_dosversion()
{
	...
	//BDD then
	ASSERT(result8==VER_MSXDOS1x || result8==VER_MSXDOS2x || result8==VER_NextorDOS, ERROR);
	SUCCEED();
}
...
void test_read_abs_sector()
{
	...
	// if this test fails, this can be useful:
	//   dos1_fcreate("boot.bin");
	//   dos1_fwrite(&buff[0], 512);
	//   dos1_fclose();

	//BDD then
	ASSERT_EQUAL(result8, 0, ERROR);
	if (dosVersion() >= VER_NextorDOS) {
		ASSERT_EQUAL(strncmp(&buff[3], "NEXTOR20", 8), 0, ERROR);
	}else{
...
void test_mapperGetTotalSegments()
{
	...
	//BDD then
	cprintf("Mapper segments: %d\r\n", result8);
	ASSERT_EQUAL(result8, 64, ERROR);
	SUCCEED();
}
...
void test_mapperGetFreeSegments()
{
	...
	//BDD then
	cprintf("Free segments: %d\r\n", result8);
	ASSERT_EQUAL(result8, mapperGetTotalSegments()-(0+7), ERROR);
	SUCCEED();
}
...
void test_dos2_findfirst()
{
	...
	if (dosVersion() >= VER_NextorDOS) {
		ASSERT_EQUAL(strcmp(ffblk->filename, "NEXTOR.SYS"), 0, ERROR);
		ASSERT_EQUAL(ffblk->attribs, ATTR_ARCHIVE, ERROR);
		ASSERT_EQUAL(ffblk->filesize, 4467, ERROR);
...
void test_dos2_findnext()
{
	...
	//BDD when
	dos2_findfirst("*.*", ffblk, ATTR_DIRECTORY);
...
void test_nxtr_get_drive_letter_info()
{
	...
	//BDD then
	ASSERT_EQUAL(result8, 0, ERROR);
	ASSERT_EQUAL(info->driveStatus, 1, ERROR); 
		// 0: Unassigned
	...
	ASSERT_EQUAL(info->driveSegment, 0xff, ERROR);	// Driver segment number, FFh if driver is embedded within a Nextor or MSX-DOS kernel ROM (always FFh in current version)
	ASSERT_EQUAL(info->relDriveNum, 0xff, ERROR);	// Relative drive number within the driver (for drive-based drivers only; FFh if device-based driver)
	ASSERT_EQUAL(info->deviceIndex, 1, ERROR);	// Device index (for device-based drivers only; 0 for drive-based drivers and MSX-DOS drivers)
	ASSERT_EQUAL(info->logicalIndex, 1, ERROR);	// Logical unit index (for device-based drivers only; 0 for drive-based drivers and MSX-DOS drivers)
	ASSERT_EQUAL(info->firstDevSec, 1, ERROR);	// First device sector number (for devices in device-based drivers only;  always zero for drive-based drivers and MSX-DOS drivers)
	SUCCEED();
}
...
void test_nxtr_get_cluster_info_fat()
{
	...
	//BDD when
	result8 = nxtr_getClusterInfoFAT(0, 2, info);
...
void test_nxtr_get_cluster_info_fat_FAILS()
{
	const char *_func = __func__;
	beforeEach();
	...
	//BDD then
	ASSERT_EQUAL(result8, ERR_ICLUS, ERROR);
	SUCCEED();
}
...
int main(char** argv, int argc)
{
	...
	if (dosVersion() >= VER_NextorDOS) {
		...
		test_nxtr_get_cluster_info_fat_FAILS();
	}
